...
function index(req, res) {
  return _sqldb.Component.findAll().then(respondWithResult(res)).catch(handleError(res));
}

// Gets a single Component from the DB
function show(req, res) {
  return _sqldb.Component.find({
    where: {
      _id: req.params.id
    }
...
function create(req, res) {
  return _sqldb.Component.create(req.body).then(respondWithResult(res, 201)).catch(handleError(res));
}
